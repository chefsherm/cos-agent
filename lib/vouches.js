...
async function readStore() {
  ...
  try {
    ...
    if (blobs.length > 0) {
      ...
      const data = JSON.parse(decrypt(text)); // handles both encrypted and legacy plaintext
...
async function writeStore(data) {
  ...
  await put(STORE, encrypt(JSON.stringify(data)), {
    access: "public",
...
export async function createRequest({ requester, contactName, contactPhone }) {
  ...
  const req = {
    ...
    // Phone numbers are used transiently to send the link and never persisted.
    hasPhone: !!(contactPhone || "").trim(),
    status: "pending",
